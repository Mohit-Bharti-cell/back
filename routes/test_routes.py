...
async def submit_test(submission: TestSubmission):
    ...
    try:  
        ...
        insert_data = {  
            ...
            "candidate_id": submission.candidate_id,  # ✅ Added candidate_id mapping
            ...
            "completed_at": datetime.now(timezone.utc).isoformat()  # ✅ Add completion timestamp
            
        }  
        # Update the existing candidate entry with test results
        # First, check if candidate exists
        existing_candidate = supabase.table("test_results").select("*").eq(
            "candidate_id", submission.candidate_id
        ).execute()
        if existing_candidate.data:
            # Update existing record
            update_result = supabase.table("test_results").update(insert_data).eq(
                "candidate_id", submission.candidate_id
            ).execute()
            print("💾 Updated existing candidate record:", update_result.data[0] if update_result.data else "No data returned")
            if update_result.data:
                result["result_id"] = update_result.data[0].get("id")
        else:
            # If no existing candidate record, insert new one
            # This shouldn't happen if login was done properly, but handle it gracefully
            insert_result = supabase.table("test_results").insert(insert_data).execute()
            print("💾 Created new record (candidate not found):", insert_result.data[0] if insert_result.data else "No data returned")
            if insert_result.data:
                result["result_id"] = insert_result.data[0].get("id")
    ...
    return {  
        ...
        "candidate_id": submission.candidate_id  # ✅ Return candidate_id in response
    } ''' 
@router.post("/submit")
async def submit_test(submission: TestSubmission):
    print("📨 Received test submission:", submission.dict())
